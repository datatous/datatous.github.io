...
excerpt: "텍스트/스크립트를 다루다 반복적으로 재발할 수 있는 두 가지 함정 — ① PowerShell 5.1의 BOM 없는 스크립트 인코딩 오독, ② 정규식 \\s가 개행을 포함해 줄 단위 정제 시 다음 줄을 통째로 삼키는 문제. 음성 출력용 텍스트 정제 스크립트를 만드는 과정에서 둘 다 실제로 발생했다."
---
...
## Connections
- → [로컬 음성 에이전트 파이프라인 구성 패턴 (Windows)](/wiki/concept-local-voice-agent-pipeline/) : 두 함정 모두
