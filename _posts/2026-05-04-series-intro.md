---
title: "나는 게을러서 자동화한다 : 2주마다 하나씩, 그 기록을 남깁니다"
date: 2026-05-04
categories:
  - 자동화연대기
tags:
...
> "이거 또 해야 해? 지난주에도 똑같이 했는데."

이 생각이 딱 드는 순간, 그냥 없애버리기 시작했다.
...
클라이언트한테 "이걸 자동화하면 좋아요"라고 말하면서 정작 내 반복 업무는 여전히 손으로 하고 있었다. 앞뒤가 안 맞았다. 좀 찔렸다. 그래서 시작했다, 직접 만들어보면서 제대로 이해해보자고.
...
그래서 지하철에서도 개발할 수 있는 환경을 먼저 세팅했다. Happy 앱에 Claude Code랑 Git 레포를 연결해서 폰으로도 코드 짜고 커밋할 수 있게 만든 거다. 이게 이 시리즈의 시작점이다. (지금은 Claude의 Remote Control 기능으로 변경해서 사용 중이다. 이 내용은 다음 편에서 정리해본다.)
...
총 8편, 약 4개월 생각하고 있다. (아래 내용은 변경될 수 있다)

| 편 | 주제 |
|---|---|
| 1편 | 출퇴근 3시간을 개발 시간으로 — Remote Control + Git 세팅 |
| 2편 | AI가 매주 교회 주보를 올려준다 — 제미나이 Gems + 클로드 에이전틱 자동화 |
| 3편 | Claude가 내 블로그를 대신 써준다? — MS 업데이트 자동 발행기 |
| 4편 | 결혼 준비를 AI와 함께 — 예산·할 일·참석자 자동화 |
| 5편 | 카카오톡에 AI를 붙인다 — MCP로 메신저 비서 만들기 |
